...
module.exports = ({
  dt = 1,
  params = { bodyMass: 60 },
  state = { compartments: [undefined, undefined], I: 0 },
} = {}) => {
  ...
  const compartments = Array(2).fill().map((e, i) => Compartment(
    { dt, state: state.compartments[i] },
  ));
  const action = Action({ dt, state: state.action });
